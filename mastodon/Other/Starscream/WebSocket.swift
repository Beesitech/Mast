...
open class WebSocket : NSObject, StreamDelegate, WebSocketClient, WSStreamDelegate {
    ...
    */
    ...
    private func createHTTPRequest() {
        ...
        var port = url.port
        if port == nil {
            if let test = url.scheme {
            if supportedSSLSchemes.contains(test) {
                port = 443
            } else {
                port = 80
            }
        ...
        guard let ho = url.host, let po = port else { return }
        
        let hostValue = request.allHTTPHeaderFields?[headerWSHostName] ?? "\(ho):\(po)"
